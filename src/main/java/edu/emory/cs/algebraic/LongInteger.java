...
public class LongInteger extends SignedNumeral<LongInteger> implements Comparable<LongInteger>{
     ...
     * @throws InvalidParameterException when `n` contains non-digit character except for the first character that can be [+-\d].
     */
    public void set(String n){
        ...
        for(int i = 0, j = n.length() - 1; i < n.length(); i++, j--) {
            byte v = (byte)(n.charAt(i) - 48); //gets ASCII value of n.charAt(i)
            digits[j] = v;
            if (0 > v || v > 9) {
    ...
    protected void addSameSign(LongInteger n) {
        ...
        //set this.digits, trims the most significant digits if it is 0
        digits = result[m] == 0 ? Arrays.copyOf(result, m) : result;

    }

    /**
     * Adds the specific integer has a different sign as this integer.
    ...
    public void multiply(LongInteger n) {
        ...
        //set this.digits, trims most significant digit if it is 0
