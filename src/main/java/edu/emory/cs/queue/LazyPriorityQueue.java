...
public class LazyPriorityQueue<T extends Comparable<T>> extends AbstractPriorityQueue<T> {
    private final List<T> keys; //defines a list to store input keys
    ...
    public LazyPriorityQueue(Comparator<T> priority) {
        super(priority); //have to initialize priority
        keys = new ArrayList<>();
    }
    ...
    public T remove() {
        ...
        T max = Collections.max(keys, priority); //find max
        keys.remove(max); //remove max
        return max;
    }
}